...
while (true) {
    ...
    for char in string {
        if char == "[" || char == "(" {stack.append(char)}
            ...
            if char == "]" && stack.removeLast() == "(" {bool = false; break}
            else if char == ")" && stack.removeLast() == "[" {bool = false; break}
        }
    }
    
    if bool == false {bools.append("no")}
    else if !stack.isEmpty {bools.append("no")}
    else {bools.append("yes")}
